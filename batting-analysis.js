document.addEventListener('DOMContentLoaded', () => {
  ...
  for (const game of games) {
    const monthIndex = new Date(game.date).getMonth(); // 0〜11
    if (monthIndex < 3) continue; // 1〜3月（4月未満）は除外
  ...
  function renderMonthlyCards(containerId) {
    ...
    validMonths.forEach(month => {
      ...
      card.className = "bg-white p-4 rounded-2xl shadow border divide-y divide-gray-200";
